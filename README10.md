...
Users: Represent individuals or applications that interact with AWS. Each user can have their own credentials. 
...
On selecting policies, click create policy and specify permissions.
Here, permissions for an EC2 user are selected. This policy is meant for Developers only as they would need access to ec2 server instances to deploy thier codes on the remote servers hosted on AWS. 
...
The same steps explained above are then also used to create another policy for the Data Analyst team. 
...
Navigate to the IAM dashboard. Under Access management, select User group.
Specify your group name and then select the preferred already created policy to link with the group. 
...
The same process is also followed to create another group for the Analyst Team.
...
Provide the name of the user. Ensure that the user can access AWS Management Console; if not selected, the user won't be able to log in from the web browser.
...
Select the permission option and assign the user to a specified group.
Then Click Create.

![](/Img10/add.png)

The same process is repeated for Mary, a data analyst user given permission to only the S3 services.
...
Login into the AWS console with aacount ID (linked wih the root user), name and the password generated when creating the user John. 
...
Provide a MFA device name and select a preferred device type from the options. In this case, the Authentication app was leveraged. 

![](/Img10/z.png)

To complete the process, follow the on-screen instructions. 
...
The above steps are also taken to enable MFA for user Mary. 
